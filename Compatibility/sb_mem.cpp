...
{
    if(!biosLoaded){  //Skip loading BIOS a second time.  We are here to load user_nvram only
...
//OpenEmu:  LoadNVmem function to load the user saved flash
void LoadNVmem(const string& root) {
    if (!sys_nvmem.Load(root, ROM_PREFIX, "nvmem.bin", "nvmem")) {
